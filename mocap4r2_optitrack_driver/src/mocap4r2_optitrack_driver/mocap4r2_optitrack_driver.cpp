...
OptitrackDriverNode::get_optitrack_system_latency(sFrameOfMocapData *data) {
  ...
  if (bSystemLatencyAvailable) {
    ...
    return round<std::chrono::nanoseconds>(
...
void OptitrackDriverNode::process_frame(sFrameOfMocapData *data) {
  ...
  if (mocap4r2_skeleton_pub_->get_subscription_count() > 0) {
    ...
    for (int i = 0; i < data->nSkeletons; i++) {
      sSkeletonData &sk_data = data->Skeletons[i];
      mocap4r2_msgs::msg::Skeleton sk_msg;
      sk_msg.skeleton_name = std::to_string(sk_data.skeletonID);
      ...
      for (int j = 0; j < sk_data.nRigidBodies; j++) {
        ...
        rb.pose.orientation.x = rb_data.qx;
...
OptitrackDriverNode::on_configure(const rclcpp_lifecycle::State &state) {
  ...
  mocap4r2_skeleton_pub_ = create_publisher<mocap4r2_msgs::msg::Skeletons>(
      "skeletons", rclcpp::QoS(1000));
